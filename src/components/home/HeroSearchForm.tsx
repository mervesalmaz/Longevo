...
const quickChips: { label: string; href: string }[] = [
  { label: "İstanbul", href: "/search?city=istanbul" },
  { label: "Ankara", href: "/search?city=ankara" },
  { label: "IV Terapi", href: "/search?treatment=iv-terapi" },
  { label: "Biyobelirteç", href: "/search?treatment=biyobelirtec" },
  { label: "NAD+ Terapi", href: "/search?treatment=nad-terapi" },
  { label: "Tüm tedaviler", href: "/search" },
];
...
export function HeroSearchForm() {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="mb-5">
        ...
        <div className="relative">
          ...
          <Input
            id="hero-search"
            type="search"
            aria-label="Klinik, tedavi veya şehir ara"
      ...
      {/* Quick filter chips */}
      <nav aria-label="Popüler aramalar" className="flex flex-wrap gap-2">
        {quickChips.map((chip) => (
          <Link
            key={chip.label}
            href={chip.href}
